...
namespace DAL
{
    public class DeptDAL
    {
        ...
        /// <summary>
        /// This method Checks whether Current Dept is used in Employee in Database or not.
        /// </summary>
        /// <param name="DeptID">Object Containing Data Values.</param>
        /// <returns>Boolean value True if Current Record already exists
        /// otherwise returns False indicating current Record Does not exist.</returns>
        public static bool IsDeptUsed(long DeptID)
        {
            bool IsRecordExist = false;
            using (SqlConnection Conn = new SqlConnection(General.GetSQLConnectionString()))
            {
                try
                {
                    ...
                    objCmd.Parameters.AddWithValue("@dbID", DeptID);
